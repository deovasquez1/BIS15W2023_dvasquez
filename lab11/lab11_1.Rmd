...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(alpha = 0.9, color = "black", fill = "gray", bins=150)+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x=log10.hra)) +
  #geom_histogram(fill = "deepskyblue4", alpha = 0.4, color = "black")+
  geom_density(color = "darkorchid4", fill = "deepskyblue4", alpha = 0.4)+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_density(color = "darkorchid4", alpha = 0.4)+
  labs(title = "Distribution of Body Mass")
```
...
homerange <- homerange %>% 
             mutate(range_category = case_when(log10.hra <= 3.653 ~ "very small",
                                               log10.hra > 3.653 & log10.mass <= 4.595 ~ "small",
...
```{r}
homerange %>% 
  group_by(class, range_category) %>% 
  count(class, range_category, sort = T)
```
```{r}
homerange %>% 
  group_by(class, range_category) %>% 
  count(class, range_category, sort = T) %>% 
  ggplot(aes(x=range_category, y=n, fill=class))+
  geom_col(position = "dodge")+
  labs(title = "Taxonomic class in each Range", x= "Range Category", y="Observations")
```
...
homerange %>% 
  ...
  ggplot(aes(x=class, y=log10.mass, fill = class))+
  geom_boxplot()+
  labs(title= "Distribution of Masses between Taxonomic Class", x= "Class", y="Log 10 Mass")
```
