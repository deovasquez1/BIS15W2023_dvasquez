...
New object is created. Use `lapply` to `read_csv` on all the `files`. They are treated like vectors. 
```{r, message = FALSE}
...
```{r}
names_vector <- unlist(names_list)
```

```{r}
names(spider_list) <- names_vector
names(spider_list)
```
...
Right join specfies the right table only (table_B) and the matching values in table_B is only joined in table_A 
```{r}
