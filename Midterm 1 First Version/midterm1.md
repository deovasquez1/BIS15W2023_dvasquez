...
##  2 Broad-winged Hawk   Buteo platypterus                1700000 TRUE            
##  3 Cooper's Hawk       Accipiter cooperii                700000 TRUE            
...
## 10 Northern Harrier    Circus cyaneus                    700000 TRUE            
## 11 Red-shouldered Hawk Buteo lineatus                   1100000 TRUE            
## 12 Red-tailed Hawk     Buteo jamaicensis                2000000 TRUE            
## 13 Rough-legged Hawk   Buteo lagopus                     300000 TRUE            
## 14 Sharp-shinned Hawk  Accipiter striatus                500000 TRUE            
## 15 Short-tailed Hawk   Buteo brachyurus                      NA NA              
## 16 Snail Kite          Rostrhamus sociabilis                 NA NA              
## 17 Swainson's Hawk     Buteo swainsoni                   540000 TRUE            
...
```r
new_eagles %>% 
  filter(!grepl(TRUE, conservation_status) & !grepl(F, conservation_status))
```
...
```
These eagles have 'NA' in population size.
...
ecosphere %>% 
  group_by(habitat) %>% 
  summarise(mean_mass = mean(log10_mass, na.rm = T),
            max_mass = max(log10_mass, na.rm = T),
            min_mass = min(log10_mass, na.rm = T),
            n_birds = n())
```

```
## # A tibble: 7 × 5
##   habitat   mean_mass max_mass min_mass n_birds
##   <chr>         <dbl>    <dbl>    <dbl>   <int>
## 1 Grassland      2.07     3.36     1.11      36
## 2 Ocean          2.72     3.54     1.63      44
## 3 Shrubland      1.59     3.28     0.48      82
## 4 Various        1.84     3.93     0.7       45
## 5 Wetland        2.56     4.04     0.96     153
## 6 Woodland       1.60     3.76     0.48     177
## 7 <NA>           1.96     2.5      1.33      14
```
