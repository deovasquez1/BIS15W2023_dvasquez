...
msleep24 <- sleep %>% 
  ...
  filter(sleep_total_24 <= 0.3 & !is.na(sleep_total_24)) %>% 
  filter(!is.na(vore)) %>% 
...
msleep %>% 
  group_by(order) %>% 
  summarise(n_vore=n_distinct(vore)) %>% 
  filter(n_vore > 1)
...
```{r}
msleep %>% 
      summarize(mean_sleep_total_lg = mean(sleep_total), 
              min_sleep_total_lg = min(sleep_total),
              max_sleep_total_lg = max(sleep_total),
              total = n())
```
...
None of the animals in cetacea have a brainwt value
