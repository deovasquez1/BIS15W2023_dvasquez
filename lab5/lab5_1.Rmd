...
glimpse(mammals)
```
Order, family, and genus should be represented as factors because they have repeated 'names'.
...
fish %>%
  ...
  filter(lakeid == "AL")
```
Takes the fish data and pipe to the select command. Then pipe the results to the filter command.
...
mammals %>% 
  ...
  arrange(newborn)
```
Note: only change -999.00 to NA. If it's not -999.00, keep the value.
