...
Shift from wide format to long format because "a,b,c,d" are drugs, and therefore their own data. 
...
```
Not violating the rules of data format, but this format allows us to make plots in R. Plotting does not work on wide data. 
...
The incomes themselves are data but do not have their own values. Therefore, we need to convert the incomes (as variables) to values. Wide format to long format. 
