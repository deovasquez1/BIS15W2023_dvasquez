...
"Type in `shinyapp` and press tab to give the skeleton
...
names(penguins)
```
Select the input that are continuous. 
Then give a variable (i.e 'x').
"Select x variable"
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    ggplot(data = homerange, aes_string(x = input$x, fill=input$y, ))+
      geom_bar(na.rm = T, position = "dodge", alpha=0.8)+
      theme_light(base_size = 18)+
      labs(x=NULL, y=NULL, fill="Fill Variable")+
      ggtitle(paste("Bar graph of Locomotion vs",input$y))
...
ui <- fluidPage(
  radioButtons("x", "Select Fill Variable", choices = c("trophic.guild", "thermoregulation"), selected = "thermoregulation"),
    plotOutput("plot", width="500px", height="400px")
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    ggplot(data = homerange, aes_string(x = "locomotion", fill=input$x, ))+
      geom_bar(na.rm = T, position = "dodge", aplha=0.8)+
      theme_light(base_size = 18)+
      labs(x=NULL, y=NULL, fill="Fill Variable")+
      ggtitle(paste("Mass of",input$x, "Density plot"))
