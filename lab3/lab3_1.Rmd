...
```{r}
w <- hbirds$weight_oz
w #Makes a new vector. Use "$" to access elements
mean(w)
```
...
```{r}
vect_temp <- hsprings$temp_C
vect_temp
avg_temp <- mean(vect_temp)
avg_temp
```

```{r}
vect_depth <- hsprings$depth_ft
vect_depth
avg_depth <- mean(vect_depth)
avg_depth
```


```{r}
average_values <- c("Averages",'',avg_temp,avg_depth )
average_values
```

```{r}
final_hsprings <- rbind(hsprings, average_values)
final_hsprings
```
...
```{r}
write.csv(final_hsprings, "hot_springs_data.csv", row.names = FALSE)
```
