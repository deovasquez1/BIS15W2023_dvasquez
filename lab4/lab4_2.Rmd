...
## Using `filter()` with multipfle observations  
...
```{r}
upd_mammals <- select(mammals, "family", "genus", "species", "mass", "gestation")
upd_mammals
```

5. Use the output from #4 to focus on the family Bovidae with a mass greater than 450000.
```{r}
filter(upd_mammals, family == "Bovidae", mass > 450000)
```
...
```{r}
best_mammals <- filter(mammals, genus == c("Lophocebus", "Erythrocebus", "Macaca"))
best_mammals
```
