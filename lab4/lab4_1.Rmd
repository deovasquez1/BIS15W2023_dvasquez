...
```
lakeid and annnumber:
*put in quotes to identify that the variables are not objects. if you don't have quotes, it may lead to errors. 
They're variables within a data frame.
```{r}
...
fish
```
*Use a colon to select a certain range of variables
```{r}
...
```{r}
new_mammals <- rename(mammals, genus = "Genus", afr = "AFR", wean_mass = "wean mass", max_life = "max. life", litter_size = "litter size", litters_year = "litters/year")
```
...
```{r}
select(new_mammals, "genus", "species", "mass")
```
